...
def extraer_pdf(pdf_bytes, filename):
    try:
        with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
            ...
            # Grupos/buses
            grupos = []
            buses = re.findall(r'BUS\s+(\d+)[^•\n]*?Hora[:\s]+([0-9_.:\-hHaAmM]+)', texto, re.IGNORECASE)
            if buses:
                ida_sec = re.search(r'IDA.*?(?=REGRESO|$)', texto, re.DOTALL|re.IGNORECASE)
                reg_sec = re.search(r'REGRESO.*', texto, re.DOTALL|re.IGNORECASE)
                buses_ida = re.findall(r'BUS\s+(\d+)[^•\n]*?Hora[:\s]+([0-9_.:\-hHaAmM]+)',
                                       ida_sec.group(0) if ida_sec else "", re.IGNORECASE)
                buses_reg = re.findall(r'BUS\s+(\d+)[^•\n]*?Hora[:\s]+([0-9_.:\-hHaAmM]+)',
                                       reg_sec.group(0) if reg_sec else "", re.IGNORECASE)
                bd = {}
                for num, hora in buses_ida:
                    n = int(num)
                    if n not in bd: bd[n] = {"grupo":str(n),"hora_salida":"","hora_llegada":"","destino":""}
                    bd[n]["hora_salida"] = parse_hora(hora)
                for num, hora in buses_reg:
                    n = int(num)
                    if n not in bd: bd[n] = {"grupo":str(n),"hora_salida":"","hora_llegada":"","destino":""}
                ...
                grupos = [{"grupo":"1","hora_salida":h_sal,"hora_llegada":h_reg,"destino":dest}]
...
def hojas_a_filas(hojas):
    ...
    for h in hojas:
        ...
        for g in h.get("grupos",[{"grupo":"1","hora_salida":"","hora_llegada":"","destino":""}]):
            filas.append({"Archivo":h.get("archivo",""),"Nº Hoja":h.get("nº_hoja",""),
                          "Fecha":h.get("fecha",""),"Cliente":h.get("cliente",""),
                          "Descripción":h.get("descripcion",""),"Grupo":g.get("grupo","1"),
                          "Hora Salida":g.get("hora_salida",""),"Hora Llegada":g.get("hora_llegada",""),
                          "Destino":g.get("destino",""),"Notas":""})
...
def generar_excel(df, titulo="Datos"):
    ...
    anchos = {"Archivo":18,"Nº Hoja":10,"Fecha":12,"Cliente":28,"Descripción":35,
              "Grupo":7,"Hora Salida":12,"Hora Llegada":12,"Destino":28,"Notas":30,
...
def main():
    ...
    archivos = st.file_uploader("Selecciona uno o varios PDFs", type=["pdf"], accept_multiple_files=True)
    ...
    with col1:
        st.download_button("⬇️ Excel con datos extraídos",
            data=generar_excel(df_ed, "Hojas de Ruta"),
    ...
    filtro = st.radio("Mostrar:", ["Todos","Solo pendientes (⚠️ y ❌)","Solo diferencias (❌)"], horizontal=True)
    if   filtro == "Solo pendientes (⚠️ y ❌)": df_v = df_verif[df_verif["Estado"].str.contains("⚠️|❌")]
    elif filtro == "Solo diferencias (❌)":      df_v = df_verif[df_verif["Estado"].str.contains("❌")]
    else:                                         df_v = df_verif
    st.dataframe(df_v, use_container_width=True, hide_index=True)

    st.download_button("⬇️ Excel de verificación",
