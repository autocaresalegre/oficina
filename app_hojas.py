...
def _parsear_hoja(texto, nro_hoja, filename):
    """Extrae los datos de una hoja de ruta individual."""
    try:
        lines = texto.split('\n')
        ...
        m_plz = re.search(r'[Nn]úmero de pa[sz]ajeros[:\s]+(\d+)', texto)
        m_npl = re.search(r'[Nn]º\s*plazas\s*totales[:\s_]*(\d+)', texto)
        m_npl2 = re.search(r'[Nn]úmero de plazas[:\s]+(\d+)', texto)
        m_prs = re.search(r'(\d+)\s*personas', texto, re.IGNORECASE)
        if m_pax:    plazas_global = m_pax.group(1)
        elif m_plz:  plazas_global = m_plz.group(1)
        elif m_npl:  plazas_global = m_npl.group(1)
        elif m_npl2: plazas_global = m_npl2.group(1)
        elif m_prs:  plazas_global = m_prs.group(1)
        ...
        if buses_check:
            ...
            for num, plazas, hora, destino in buses_reg:
                n = int(num)
                if n not in bd: bd[n] = {"grupo":str(n),"plazas":plazas,"hora_salida":"","hora_llegada":"","destino":"","destino_regreso":""}
                bd[n]["hora_llegada"] = parse_hora(hora); bd[n]["destino_regreso"] = limpiar(destino)[:60]
            ...
            if not m_s: m_s = re.search(r'[Hh]orario de salida[:\s]+(\d{1,2}[:.:]\d{2})', texto)
            if not m_r: m_r = re.search(r'[Hh]orario de regreso[:\s]+(\d{1,2}[:.:]\d{2})', texto)
            if m_s:
                h_sal = parse_hora(m_s.group(1))
                try: dest = (m_s.group(2) or "").strip()[:60]
                except: dest = ""
            if m_r: h_reg = parse_hora(m_r.group(1))
...
def hojas_a_filas(hojas_anidadas):
    ...
    # Aplanar: extraer_pdf ahora devuelve lista de hojas por PDF
    hojas = []
