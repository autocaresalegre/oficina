...
def _parsear_hoja(texto, nro_hoja, filename):
    ...
    try:
        ...
        # Observaciones: resumen de las primeras líneas útiles
        obs = " | ".join(desc_lines[:4])[:160] if desc_lines else ""
        ...
        for pat in [
            ...
            if m:
                d = re.sub(r'\s*\([^)]{0,40}\)$','', m.group(1).strip())
                if not d.startswith('http') and len(d) > 3:
                    destino_global = d.strip()[:60]; break
        # Patrón "[Lugar] Salida HH:MM" en misma línea o línea anterior
        ...
        for pat in [
            ...
            r'[Pp]lazas[:\s]+(\d+)',
            r'[Aa]lumnos[:\s]+(\d+)',
            r'(\d+)\s+(?:[Aa]lumnos|[Pp]asajeros|[Pp]ersonas|[Pp]laces?)\b',
            r'(\d+)\s*(?:PAX|pax|Pax)\b',
            r'(\d+)pax\b',
            r'(\d+)\s+seater',
            r'(\d+)\s+persones',
