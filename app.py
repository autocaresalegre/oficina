...
def formulario_conductor(prefix=""):
    ...
    nombre_sel=st.selectbox("👤 Selecciona tu Nombre",
                            options=["— Selecciona —"]+conductores,
                            key=f"{prefix}nombre",
                            help="Escribe las primeras letras para filtrar")
...
def vista_admin():
    ...
    if not df_brutos.empty:
        ...
        if not df_cierre.empty:
            cols_m=["Conductor","Fecha","Servicios Fijos","Por Horas",
                    "IMP. FIJOS","IMP. HORAS","IMP. DIETAS","TOTAL","REVISAR"]
            st.dataframe(df_cierre[[c for c in cols_m if c in df_cierre.columns]],
                         use_container_width=True,hide_index=True,
                         column_config={
                             "IMP. FIJOS":st.column_config.NumberColumn(format="%.2f EUR"),
                             "IMP. HORAS":st.column_config.NumberColumn(format="%.2f EUR"),
                             "IMP. DIETAS":st.column_config.NumberColumn(format="%.2f EUR"),
                             "TOTAL":st.column_config.NumberColumn(format="%.2f EUR"),
                         })
            m1,m2,m3,m4=st.columns(4)
            m1.metric("Fijos",f"{df_cierre['IMP. FIJOS'].sum():,.2f} EUR")
            m2.metric("Horas",f"{df_cierre['IMP. HORAS'].sum():,.2f} EUR")
            m3.metric("Dietas",f"{df_cierre['IMP. DIETAS'].sum():,.2f} EUR")
            m4.metric("TOTAL",f"{df_cierre['TOTAL'].sum():,.2f} EUR")
            st.markdown("---")
            c_xl,c_pdf=st.columns(2)
            nombre_arch=f"autocares_{date.today().strftime('%Y_%m_%d')}"
            with c_xl:
                st.download_button("Excel",data=generar_excel_bytes(df_cierre),
                    file_name=f"{nombre_arch}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    type="primary",use_container_width=True)
            with c_pdf:
                st.download_button("PDF",data=generar_pdf_bytes(df_cierre),
