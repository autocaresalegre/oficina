...
COLUMNAS_BD = ["conductor","fecha","dieta","servicios_fijos","servicios_horas","observaciones","hoja_servicio"]
...
def formulario_conductor(prefix=""):
    ...
    nombre_sel=st.selectbox("👤 Selecciona tu Nombre",
                            ...
                            key=f"{prefix}nombre")
    # Si el conductor no aparece, mostrar campo de texto libre
    if nombre_sel == "✏️ Mi nombre no aparece en la lista":
        ...
        if nombre_manual.strip():
            ...
            nombre_sel = "— Selecciona —"  # Forzar validación si está vacío
    ...
    st.markdown("---")
    st.markdown("---")
    nro_servicio=st.number_input("📋 Nº Servicio (opcional)",min_value=0,max_value=99999,
                                  value=0,step=1,key=f"{prefix}nro_servicio",
                                  help="Número de hoja de servicio. Deja en 0 si no lo sabes.")
    st.markdown("---")
    ...
    return nombre_sel,fecha_sel,conteo,horas_cap,dieta,dieta_conc,obs,nro_servicio
...
def vista_conductor():
    ...
    st.divider()
    nombre_sel,fecha_sel,conteo,horas_cap,dieta_sel,dieta_conc_sel,obs,nro_servicio_sel = formulario_conductor("c_")
    st.divider()
    if st.button("VER RESUMEN Y CONFIRMAR",type="primary",use_container_width=True):
        ...
        if errores:
            ...
            # Si hay concepto de dieta, añadirlo al inicio de observaciones
            obs_final = obs.strip()
            ...
            st.session_state.datos_resumen={
                ...
                "nro_servicio":int(nro_servicio_sel)}
            st.rerun()
...
def vista_admin():
    ...
    with st.expander("Anadir Registro Manual"):
        nombre_m,fecha_m,conteo_m,horas_m,dieta_m,dieta_conc_m,obs_m,nro_servicio_m = formulario_conductor("m_")
