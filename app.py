...
def formulario_conductor(prefix=""):
    ...
    busqueda=st.text_input("🔍 Escribe tu nombre o DNI",
                          placeholder="Escribe al menos 2 letras o tu DNI...",
                          key=f"{prefix}busqueda")
    # Solo mostrar coincidencias si hay al menos 2 caracteres escritos
    termino = busqueda.strip().lower()
    if len(termino) >= 2:
        cond_f = [c for c in conductores if termino in c.lower()]
    else:
        cond_f = []
    opciones = ["— Escribe tu nombre arriba —"] + cond_f + ["✏️ Mi nombre no aparece en la lista"]
    ...
    if nombre_sel == "✏️ Mi nombre no aparece en la lista":
        ...
        if nombre_manual.strip():
            ...
            nombre_sel = "— Escribe tu nombre arriba —"
    # Normalizar el valor vacío
    if nombre_sel == "— Escribe tu nombre arriba —":
...
def vista_admin():
    ...
    if not df_brutos_raw.empty:
        ...
        df_brutos_periodo = df_brutos_raw[
    ...
    if not df_brutos_periodo.empty:
        conductores_lista = ["Todos"] + sorted(df_brutos_periodo["conductor"].dropna().unique().tolist())
        filtro_cond = st.selectbox("👤 Filtrar por conductor:", conductores_lista, key="filtro_cond_registros")
        if filtro_cond != "Todos":
            df_brutos = df_brutos_periodo[df_brutos_periodo["conductor"] == filtro_cond].copy()
        else:
            df_brutos = df_brutos_periodo
    else:
        df_brutos = df_brutos_periodo
    if df_brutos.empty:
    ...
    if not df_brutos_periodo.empty:
        # Calcular siempre sobre todos los datos del periodo (no el filtrado)
        df_cierre = calcular_liquidacion(df_brutos_periodo)
        if not df_cierre.empty:
            # Filtro conductor para importes
            conductores_imp = ["Todos"] + sorted(df_cierre["Conductor"].unique().tolist())
            filtro_cond_imp = st.selectbox("👤 Filtrar por conductor:", conductores_imp, key="filtro_cond_importes")

            # Recalcular si cambian los datos, pero conservar ediciones manuales
            clave_datos = str(len(df_brutos_periodo))
            ...
            if filtro_cond_imp != "Todos":
                df_para_editor = df_para_editor[df_para_editor["Conductor"] == filtro_cond_imp]
            ...
            # ── Filtro de conductores para Excel y PDF ───────
            todos_c = sorted(df_ed["Conductor"].unique().tolist())
            sel_export = st.multiselect(
                "📄 Seleccionar conductores para Excel/PDF (vacío = todos):",
                options=todos_c,
                default=[],
            ...
            df_para_export = df_base_export[df_base_export["Conductor"].isin(sel_export)] if sel_export else df_base_export

            # ── Botones descarga ─────────────────────────────
            nombre_arch = f"autocares_{date.today().strftime('%Y_%m_%d')}"
            c_xl,c_pdf=st.columns(2)
            with c_xl:
                if not df_para_export.empty:
                    st.download_button("⬇️ Excel",
                        data=generar_excel_bytes(df_para_export),
                        file_name=f"{nombre_arch}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        type="primary", use_container_width=True)
                else:
                    st.warning("Selecciona al menos un conductor.")
            with c_pdf:
                if not df_para_export.empty:
                    st.download_button("📄 PDF",
                        data=generar_pdf_bytes(df_para_export),
