...
def formulario_conductor(prefix=""):
    ...
    return nombre_sel,fecha_sel,conteo,horas_cap,dieta,dieta_conc,obs
...
def vista_conductor():
    ...
    st.divider()
    nombre_sel,fecha_sel,conteo,horas_cap,dieta_sel,dieta_conc_sel,obs = formulario_conductor("c_")
    st.divider()
...
def vista_admin():
    ...
    if not df_brutos.empty:
        ...
        if not df_cierre.empty:
            ...
            df_para_pdf = st.session_state.df_cierre_editado[st.session_state.df_cierre_editado["Conductor"].isin(sel_pdf)] if sel_pdf else st.session_state.df_cierre_editado
            ...
            with c_xl:
                st.download_button("⬇️ Excel",
                    data=generar_excel_bytes(st.session_state.df_cierre_editado),
    ...
    with st.expander("Anadir Registro Manual"):
        nombre_m,fecha_m,conteo_m,horas_m,dieta_m,dieta_conc_m,obs_m = formulario_conductor("m_")
        if st.button("Guardar Registro Manual",type="primary",use_container_width=True,key="btn_manual"):
            if nombre_m=="— Selecciona —":
                ...
                if guardar_fila(fila):
                    ...
                    # Limpiar todos los campos del formulario manual
                    for k in list(st.session_state.keys()):
                        if k.startswith("m_"):
